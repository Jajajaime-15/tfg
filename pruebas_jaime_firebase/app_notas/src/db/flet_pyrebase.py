...
class PyrebaseWrapper:
    ...
    def __init__(self, page):
        ...
        #self.check_token() # esto no va bien aqui, mejor lo llamamos desde el main

        self.streams = []
        self.prefs = ft.SharedPreferences()

    async def save_tokens(self, token, uuid):
        encrypted_token = encrypt(token, secret_key)
        await self.prefs.set("firebase_token", encrypted_token)
        await self.prefs.set("firebase_id", uuid)
        self.idToken = token
        self.uuid = uuid

    async def erase_token(self):
        await self.prefs.remove("firebase_token")
        await self.prefs.remove("firebase_id")
    ...
    async def check_token(self):
        encrypted_token = await self.prefs.get("firebase_token")
        uuid = await self.prefs.get("firebase_id")
        if encrypted_token:
